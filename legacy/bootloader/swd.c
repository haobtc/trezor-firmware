...
void swd_clock_cycle(void)  //һ��ʱ������   ����ʱ�ӣ��ӳ٣�����ʱ�ӣ���ʱ
{
  clr_swd_clk();
  Delay_n_us(2);
  set_swd_clk();
...
void swd_write_bit(
    unsigned char b)  //�������Ϸ���һλ :����ʱ�ӣ��������ݿ�״̬����ʱ������ʱ�ӣ���ʱ
{
  clr_swd_clk();
  if (b & 0x01) {
    set_swd_sda();
...
unsigned char swd_read_bit(
    void)  //�������Ͻ���һλ������ʱ�ӣ���ʱ��ȡ������״̬������ʱ�ӣ���ʱ
{
  unsigned char b = 0;
  clr_swd_clk();
  Delay_n_us(2);
...
static short swd_send(unsigned char *in, unsigned short bits)  //��������
{
  unsigned char t, j;
  unsigned short i;
  t = bits / 8;
...
static short swd_receive(unsigned char *out, unsigned short bits)  //��������
{
...
=======
  reqs = reqhdr;
  res = swd_parity_even((unsigned char *)&reqs, &parity, 8);
>>>>>>> new_boot
  if (res < 0) {
    return res;
  }
...
short swd_dap_init() {
  ...
  g_page_size = 128;  //���ٶ�ȡflashһ��дҳ��С
...
static unsigned char swd_write_block(unsigned int addr, unsigned char *p,
  ...
  for (i = 0; i < size_in_words; i++) {
    //       reverse((unsigned int *)p); //���߷�ת
    //       swd_update_crc(p,4);// ��ʹ��crc��֤
